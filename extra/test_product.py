...
def create_product():
    field_validation = (
        ('brand_id',gl.required,gl.type_(int)),
        ('product_name',gl.required,gl.type_(str)),
        ('category_id',gl.required,gl.type_(int)),
        ('model_year',gl.required,gl.type_(int)),
        ('list_price',gl.required,gl.type_(int))
    )
    try:        
        _json = request.json
        ...
        result = gl.validate(field_validation,_json)
        print(type(result))
        print(result)
        # return result
        # for i in result:
        ...
        if result and request.method == 'POST':
            ...
            # return showMessage()
            return "Error due to invalid input or wrong HTTP method selection"
    except Exception as e:
        # print(e)
        return 'something 2'
